...
class DBManagerTests: XCTestCase {
    ...
    func testDatabaseRecordConversion() {
        ...
        let timestamp = Date()
        let version = "123"
        let touchRecord = TouchRecord(deviceName: deviceName, timestamp: timestamp, version: version, origin: .database)
...
extension DBManagerTests {
    ...
    class MockDatabase: Database {
        ...
        func fetchRecords(sinceStartOf date: Date, completionHandler: @escaping (Result<[TouchRecord], DatabaseError>) -> Void) {
            ...
            let record = TouchRecord(deviceName: "123", timestamp: startOfDay, version: "123", origin: .database)
